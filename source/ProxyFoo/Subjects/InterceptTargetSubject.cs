...
{
    [DebuggerDisplay("InterceptTarget<{Type}>")]
    public class InterceptTargetSubject : SubjectBase, IPerSubjectCoderFactory<ISubjectMethodExistsPerSubjectCoder>
    {
        public InterceptTargetSubject(Type type) : base(type) { }

        public override void Initialize(IMixinDescriptor mixin)
        {
            if (!(mixin is IRealSubjectMixin))
                throw new InvalidOperationException("An intercept target subject must be part of a real subject mixin.");
